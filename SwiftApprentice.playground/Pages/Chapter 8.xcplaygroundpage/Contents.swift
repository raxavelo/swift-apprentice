...
/*
 ...
 For the Fibonacci numbers, you can use the function you wrote in Chapter 5, “Functions”
...
//appRatings.forEach { (name, rates) in
//  let total = rates.reduce(0) { sum, rate in
//    sum + rate
//  }
//  averageRatings[name] = Double(total) / Double(rates.count)
//}
//
//averageRatings.filter() { (_, avgRate) in
//  avgRate > 3
//}.map() { (name, _) in
//  name
//}
